...
# init variables
def init_cmatrix(region_size, mask_size):
    ...
    if P not None:
		...
		P = pyamg.gallery.poisson(mask_size)
...
def test():
    prefix = './testimages/'
    img_mask = np.asarray(PIL.Image.open(prefix + 'test1_mask.png'))
    img_mask.flags.writeable = True
    img_source = np.asarray(PIL.Image.open(prefix + 'test1_src.png'))
    ...
    img_ret = blend(img_target, img_source, img_mask, offset=(-16, -74))
